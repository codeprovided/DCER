...
def build_input_data(u_text, i_text, vocabulary_u, vocabulary_i):
    """
         Map the sentences to numbers according to the vocabulary.
    """
    u_text2 = {}
    for i in u_text.keys():
        u_reviews = u_text[i]
...
u_train = []  # train，user id
i_train = []  # train，item id
r_train = []  # train，score
...
u_text = pad_sentences(u_text, u_w_len)  # Standardize the number of words in user comments.
print("pad user done")
i_text = pad_sentences(i_text, i_w_len)  # Standardize the number of words in item comments.
print("pad item done")
...
#Use vocabulary_user to convert all words in user comments into numbers, and use vocabulary_item to convert all words in item comments into numbers.
