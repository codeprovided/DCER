...
def build_input_data(text, vocabulary):
    """
        Map sentences to numbers based on the vocabulary.
    """
    text2 = {}
    for i in text.keys():
        u_reviews = text[i]
...
# user_rid = pickle.load(open('../dataset/1_user_rid.pkl', 'rb'))
# user_rid_ra = pickle.load(open('../dataset/1_user_rid.pkl', 'rb'))

item_reviews = pickle.load(open('../dataset/ele2013/ele2013_item_review.pkl', 'rb'))
...
u_text = pad_sentences(u_text, u_len, u_w_len)  # Standardize the number of words in user comments.
# print("pad user done")
# i_text = pad_sentences(i_text, i_len, i_w_len)  # Standardize the number of words in item comments.
# print("pad item done")
#
user_voc = [xx for x in u_text.values() for xx in x]  # Expand user comments? Concatenate without labeling user IDs.
...
# u_text, i_text = build_input_data(u_text, i_text, vocabulary_user, vocabulary_item)  # Convert the words in the comments to numbers.
# para = pickle.load(open('../dataset/yelp/para.pkl', 'rb'))
...
para['u_text'] = u_text
para['i_text'] = i_text
para['user_num'] = u_text.__len__()
para['item_num'] = i_text.__len__()
para['user_length'] = u_len
para['item_length'] = i_len
para['review_size'] = u_w_len
para['user_vocab'] = vocabulary_user
para['item_vocab'] = vocabulary_item

# pickle.dump(u_text, open('../dataset/yelp/u_text_6.pkl', 'wb'))
# # pickle.dump(i_text, open('../dataset/yelp/i_text.pkl', 'wb'))
pickle.dump(para, open('../dataset/ele2013/para_ele.pkl', 'wb'))
