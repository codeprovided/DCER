...
class DCER(nn.Module):
    ...
    def forward(self, nodes_u, nodes_v, history_u_lists, history_ura_lists, history_v_lists, history_vra_lists):  # Input information.
        embeds_u = self.enc_u_history(nodes_u, history_u_lists, history_ura_lists)
        embeds_v = self.enc_v_history(nodes_v, history_v_lists, history_vra_lists)
    ...
    def loss(self, nodes_u, nodes_v, labels_list, history_u_lists, history_ura_lists,
             history_v_lists, history_vra_lists):
        scores = self.forward(nodes_u, nodes_v, history_u_lists, history_ura_lists, history_v_lists,
                              history_vra_lists)
...
def train(model, device, train_loader, optimizer, epoch, best_rmse, best_mae, history_u_lists, history_ura_lists,
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        loss = model.loss(batch_nodes_u.to(device), batch_nodes_v.to(device), labels_list.to(device), history_u_lists,
                          history_ura_lists, history_v_lists, history_vra_lists)
...
def test(model, device, test_loader, history_u_lists, history_ura_lists, history_v_lists, history_vra_lists):
    model.eval()
    tmp_pred = []
    target = []
    with torch.no_grad():
        for test_u, test_v, tmp_target in test_loader:
            ...
            val_output = model.forward(test_u, test_v, history_u_lists, history_ura_lists,
                                       history_v_lists, history_vra_lists)
...
def main():
    ...
    # user feature
    agg_u_history = UV_Aggregator(v2e, r2e, u2e, opts.embed_dim, opts.word_dim, opts.vocab_size_u, opts.num_filters,
    ...
    # item feature:
    agg_v_history = UV_Aggregator(v2e, r2e, u2e, opts.embed_dim, opts.word_dim, opts.vocab_size_v, opts.num_filters,
    ...
    # model
    dcer = DCER(enc_u_history, enc_v_history).to(device)
    ...
    dcer_best = DCER(enc_u_history, enc_v_history).to(device)
    print("load model......")
    dcer_best.load_state_dict(torch.load('../model/ge_ele.pt'))
    expected_rmse, mae = test(dcer_best, device, test_loader, history_u_lists, history_ura_lists,
                                  history_v_lists, history_vra_lists)
    print("rmse: %.4f, mae:%.4f " % (expected_rmse, mae))

    for epoch in range(1, opts.epochs + 1):

        train(dcer, device, train_loader, optimizer, epoch, best_rmse, best_mae, history_u_lists, history_ura_lists,
              history_v_lists, history_vra_lists)
        expected_rmse, mae = test(dcer, device, test_loader, history_u_lists, history_ura_lists,
                                  history_v_lists, history_vra_lists)
