...
# from DCER.UV_Aggre import UV_Aggregator
# from DCER.UV_Enco import UV_Encoder
...
def main():
    ...
    dcer_best = DCER(enc_u_history_1, enc_u_history_2, enc_u_history_3, enc_u_history_4, enc_u_history_5,
                     enc_v_history, device).to(device)
    print("load model......")
    dcer_best.load_state_dict(torch.load('../model/dcer_p_yelp.pt'))
    ...
    #         endure_count = 0
    #         print("save model")
    #         torch.save(dcer.state_dict(), '../model/dcer_p_yelp.pt')
    #     else:
    #         endure_count += 1
    #     print("epoch:", epoch)
    #     print("rmse: %.4f, mae:%.4f " % (expected_rmse, mae))
